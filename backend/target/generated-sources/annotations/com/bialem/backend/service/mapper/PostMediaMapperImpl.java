...
public class PostMediaMapperImpl implements PostMediaMapper {
    ...
    public PostMedia toEntity(PostMediaDTO dto) {
        ...
        postMedia.setId( dto.getId() );
        postMedia.setMediaType( dto.getMediaType() );
        postMedia.setStoragePath( dto.getStoragePath() );
        postMedia.setSortOrder( dto.getSortOrder() );
        postMedia.setCreatedAt( dto.getCreatedAt() );
        postMedia.post( postDTOToPost( dto.getPost() ) );

        return postMedia;
    }

    @Override
    public List<PostMedia> toEntity(List<PostMediaDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<PostMedia> list = new ArrayList<PostMedia>( dtoList.size() );
        for ( PostMediaDTO postMediaDTO : dtoList ) {
            list.add( toEntity( postMediaDTO ) );
        }
    ...
    @Override
    public List<PostMediaDTO> toDto(List<PostMedia> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<PostMediaDTO> list = new ArrayList<PostMediaDTO>( entityList.size() );
        for ( PostMedia postMedia : entityList ) {
            list.add( toDto( postMedia ) );
        }
    ...
    @Override
    public void partialUpdate(PostMedia entity, PostMediaDTO dto) {
        if ( dto == null ) {
            return;
        }
        ...
        if ( dto.getPost() != null ) {
            ...
            postDTOToPost1( dto.getPost(), entity.getPost() );
        }
    }

    @Override
    public PostMediaDTO toDto(PostMedia s) {
        if ( s == null ) {
            return null;
        }
        ...
        postMediaDTO.setPost( toDtoPostId( s.getPost() ) );
        postMediaDTO.setId( s.getId() );
        postMediaDTO.setMediaType( s.getMediaType() );
        postMediaDTO.setStoragePath( s.getStoragePath() );
        postMediaDTO.setSortOrder( s.getSortOrder() );
        postMediaDTO.setCreatedAt( s.getCreatedAt() );

        return postMediaDTO;
    }

    @Override
    public PostDTO toDtoPostId(Post post) {
        if ( post == null ) {
            return null;
        }
    ...
    protected Post postDTOToPost(PostDTO postDTO) {
        ...
        post.setModerationStatus( postDTO.getModerationStatus() );
        post.setCreatedAt( postDTO.getCreatedAt() );
        post.setUpdatedAt( postDTO.getUpdatedAt() );
        post.community( communityDTOToCommunity( postDTO.getCommunity() ) );
        post.event( eventDTOToEvent( postDTO.getEvent() ) );
        post.author( profileDTOToProfile( postDTO.getAuthor() ) );
    ...
    protected void postDTOToPost1(PostDTO postDTO, Post mappingTarget) {
        if ( postDTO == null ) {
            return;
        }

        if ( postDTO.getId() != null ) {
            mappingTarget.setId( postDTO.getId() );
        }
        if ( postDTO.getBody() != null ) {
            mappingTarget.setBody( postDTO.getBody() );
        }
        if ( postDTO.getVisibility() != null ) {
            mappingTarget.setVisibility( postDTO.getVisibility() );
        }
        if ( postDTO.getModerationStatus() != null ) {
            mappingTarget.setModerationStatus( postDTO.getModerationStatus() );
        }
        if ( postDTO.getCreatedAt() != null ) {
            mappingTarget.setCreatedAt( postDTO.getCreatedAt() );
        }
        if ( postDTO.getUpdatedAt() != null ) {
            mappingTarget.setUpdatedAt( postDTO.getUpdatedAt() );
        }
        if ( postDTO.getCommunity() != null ) {
            if ( mappingTarget.getCommunity() == null ) {
                mappingTarget.community( new Community() );
            }
            communityDTOToCommunity1( postDTO.getCommunity(), mappingTarget.getCommunity() );
        }
        if ( postDTO.getEvent() != null ) {
            if ( mappingTarget.getEvent() == null ) {
                mappingTarget.event( new Event() );
            }
            eventDTOToEvent1( postDTO.getEvent(), mappingTarget.getEvent() );
        }
        if ( postDTO.getAuthor() != null ) {
            if ( mappingTarget.getAuthor() == null ) {
                mappingTarget.author( new Profile() );
            }
            profileDTOToProfile1( postDTO.getAuthor(), mappingTarget.getAuthor() );
        }
    }
}
