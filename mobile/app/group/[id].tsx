...
export default function GroupDetailScreen() {
  ...
  const confirmLeaveGroup = async () => {
    ...
    if (leaveError) {
      ...
      void showAppError(leaveError.message);
    } else {
      void showAppSuccess("Gruptan ayrıldın.");
      router.replace({ pathname: "/community/[id]", params: { id: group.parent_id } });
    }
  };
  ...
  const confirmRemoveMember = async (member: ManagedMember) => {
    ...
    if (removeError) {
      ...
      void showAppError(removeError.message);
    } else {
      void showAppSuccess(`${member.display_name} gruptan çıkarıldı.`);
      await loadGroup("refresh");
    }
    setBusyId(null);
  };
...
const styles = StyleSheet.create({
  page: { flexGrow: 1, padding: 20, paddingBottom: 46, gap: 18, backgroundColor: colors.page },
  loadingBox: { minHeight: 360, alignItems: "center", justifyContent: "center" },
  hero: { padding: 24, gap: 10, borderRadius: 30, backgroundColor: colors.accent },
  kicker: { color: colors.action, fontSize: 11, fontWeight: "900", letterSpacing: 1.4 },
  title: { color: "#ffffff", fontSize: 35, lineHeight: 40, fontWeight: "900" },
  ...
  panel: { padding: 20, gap: 13, borderRadius: 26, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface },
  ...
  actionCopy: { gap: 5 },
  primaryButton: { minHeight: 49, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, borderRadius: 999, backgroundColor: colors.action },
