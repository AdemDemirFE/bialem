...
export default async function StoreAdminPage() {
  ...
  const ordersPage = await api.rest.get<any>("/api/store/orders/admin/all?page=0&size=1");
  const productsPage = await api.rest.get<any>("/api/store/products?page=0&size=1");
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Link href="/admin/store/products" className="p-6 rounded-xl border bg-white hover:shadow transition">
          ...
          <div className="text-2xl font-bold mt-1">{productsPage?.totalElements ?? 0}</div>
        </Link>
        <Link href="/admin/store/orders" className="p-6 rounded-xl border bg-white hover:shadow transition">
          ...
          <div className="text-2xl font-bold mt-1">{ordersPage?.totalElements ?? 0}</div>
        </Link>
      </div>
    </div>
  );
